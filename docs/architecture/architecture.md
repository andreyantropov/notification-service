...
Для удобства разработчиков и автоматизации интеграции предоставлена **полная OpenAPI-спецификация (Swagger)**, доступная в интерактивной форме по адресу `/docs`. Она включает:
...
Сервис поддерживает полную наблюдаемость через систему логирования, которая включает:

- **Структурированные логи** с тегами для фильтрации (уровень, окружение, источник, тип события).
- **Сквозную трассировку** с использованием `traceId` и `correlationId`.
- **Резервные каналы записи** (InfluxDB → файл → stdout), чтобы гарантировать, что критические события не будут потеряны.
- **Визуализацию в реальном времени** через Grafana.
- **Алертинг в Telegram** на события уровня `error` и `critical`.

Архитектура логирования следует принципу **"fail-safe logging"** — даже при отказе внешних систем логи не теряются.

> 💡 Подробная схема полей, тегов и дашбордов доступна в: [`docs/monitoring.md`](./monitoring.md).
...
Сервис спроектирован как **stateless** и может быть легко **горизонтально масштабирован** в Kubernetes или Docker Swarm.
