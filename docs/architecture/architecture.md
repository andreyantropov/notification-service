...
1. **Сервис уведомлений (NS)**
   ...
   - Зависимости:
     - Библиотеки: `axios`, `nodemailer`, `express`, `@influxdata/influxdb-client`, `dotenv`, `awilix`, `opentelemetry`, `@cloudamqp/amqp-client`.
...
flowchart LR
    ...
    NS -->|логирование| InfluxDB[(InfluxDB)]
    InfluxDB --> Grafana[Grafana / Алерты]
...
5. **Логирование**:
   - Все этапы (приём, публикация, доставка, ошибки) логируются в InfluxDB.
...
- **Прозрачная инструментация**: критические компоненты — отправители (`channels`) и буферы (`buffers`) — автоматически оборачиваются **декораторами телеметрии** при сборке приложения. Эти обёртки добавляют контекст (`traceId`, `spanId`, `durationMs`) и фиксируют события без вмешательства в основную логику.
...
- **Сквозная трассировка** через `traceId` (OpenTelemetry), что упрощает диагностику в распределённой системе.
