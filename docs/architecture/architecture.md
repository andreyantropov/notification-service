...
graph TD
  ...
  DeliveryService -->|sends via| BitrixChannel
  DeliveryService -->|sends via| SmtpChannel
  BufferImpl -.->|implements| Buffer
  BitrixChannel -.->|implements| Channel
  SmtpChannel -.->|implements| Channel
...
3. **Ответ клиенту**:
   ...
   - Срочные (`isImmediate: true`) — отправляются немедленно.
