...
- ✅ **Валидные уведомления** — будут приняты в обработку.
...
> 💡 **Пример**: вы отправили 10 уведомлений, 9 из них валидны — они будут приняты. 1 — с ошибкой email — будет отклонён, но не помешает остальным.
...
- `details` содержит:
  ...
  - `notification` — исходный объект (если валидный — `Notification`, иначе — `unknown`),
...
- точные схемы `Notification`, `Recipient`, `SendResult`, `SendResponse`,
