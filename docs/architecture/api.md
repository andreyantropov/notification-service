...
Healthcheck-эндпоинты (`/health/*`) и документация (`/api-docs`) **публичны** и не требуют аутентификации.
...
Поле `contacts` — это **список контактов одного получателя**.  
...
- `isImmediate: true` → уведомление отправляется **немедленно**.
- `isImmediate: false` или отсутствует → уведомление **буферизуется** и отправляется **пачкой** раз в `NOTIFICATION_BATCH_INTERVAL_MS` (по умолчанию — 60 секунд).

> 💡 Используйте `isImmediate` для критичных событий (например, сброс пароля, авария).  
...
- `acceptedCount` — количество уведомлений, прошедших валидацию.
- `rejectedCount` — количество уведомлений, не прошедших валидацию.
- `details` содержит:
  - `success: boolean` — прошло ли уведомление валидацию,
  - `notification` — **если валидное — объект с `id`, `contacts`, `message`, `isImmediate`; если невалидное — исходный объект из запроса**,
...
- 🕒 **Для срочных уведомлений используйте `isImmediate: true`**.
