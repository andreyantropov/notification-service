...
describe("NotificationProcessService", () => {
    ...
    describe("processNotifications", () => {
        it("should process notifications successfully when recipients are available", async () => {
            mockNotificationSource.getNotifications.mockResolvedValue([mockNotification]);
            ...
            expect(mockNotificationSource.deleteNotification).toHaveBeenCalledWith(mockNotification.id);
        });

        it("should log error and delete notification when no recipients are available", async () => {
            mockNotificationSource.getNotifications.mockResolvedValue([mockNotificationWithoutContacts]);
            ...
            expect(mockResolveRecipients).toHaveBeenCalledWith(mockNotificationWithoutContacts);
            ...
            expect(mockNotificationSource.deleteNotification).toHaveBeenCalledWith(mockNotificationWithoutContacts.id);
        });
        ...
        it("should process multiple notifications sequentially", async () => {
            ...
            mockNotificationSource.getNotifications.mockResolvedValue([mockNotification, secondNotification]);
            mockResolveRecipients.mockReturnValue(["email@example.com"]);
            ...
            expect(mockNotificationSource.deleteNotification).toHaveBeenCalledWith(mockNotification.id);
            expect(mockNotificationSource.deleteNotification).toHaveBeenCalledWith(secondNotification.id);
        });
        ...
        it("should use empty array as default recipients when resolveRecipients is not provided", async () => {
            ...
            expect(mockNotificationLogger.writeLog).toHaveBeenCalledWith(expect.objectContaining({
                level: LogLevel.Error,
                message: "Отсутствуют контакты клиента"
            }));
